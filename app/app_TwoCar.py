...
def task_press_up_handle():
    ...
    while True:
        ...
        if g_press_up:
            if state_count == 1:
                g_car.translation('z', 75)
            elif state_count == 5:
                g_car.translation('z', 100)
            elif state_count == 10:
                state_count = 0
                index = index + 1
...
def parse_data(data):
    ...
    if cmd == "0F":
        ...
        g_car.move_x(speed_x)
        g_car.move_y(speed_y)
        ...
        if num_dir == 1:
            ...
            step_Z = int(my_map(g_step_control, 0, 100, 20, 0.6*100))
            step_X = int(STEP_SCALE_X * g_step_control)
            g_car.turnleft(step_Z)  
            g_car.forward(step_X)      
            #g_car.left(step) #此数据改动一下，app结合轮足小车，不能使用机器狗 
        elif num_dir == 4:
            step_Z = int(my_map(g_step_control, 0, 100, 20, 0.6*100))
            step_X = int(STEP_SCALE_X * g_step_control)
            g_car.turnright(step_Z)
            g_car.forward(step_X)
            #g_car.right(step) #此数据改动一下，app结合轮足小车，不能使用机器狗
        elif num_dir == 5:
            step = int(my_map(g_step_control, 0, 100, 20, STEP_SCALE_Z*100))
            g_car.turnleft(step)
        elif num_dir == 6:
            step = int(my_map(g_step_control, 0, 100, 20, STEP_SCALE_Z*100))
            g_car.turnright(step)
        elif num_dir == 7:
            dogzilla_reset()
        elif num_dir == 0:
            g_car.stop()
        ...
        g_car.imu(g_car_stabilize_state)
        ...
        if num_gait > 0:
            print("walk")
            g_car.gait_type("walk")
        else:
            print("trot")
            g_car.gait_type("trot")
        ...
        if num_calibrate > 0:
            print("start")
            g_car.calibration('start')
        elif num_calibrate == 0:
            print("end")
            g_car.calibration('end')
        else:
            print("error")
        ...
        if -35 < num_x_length< 35:
            ...
            g_car.translation('x', g_x_length)
        else:
            pass
        ...
        if -35 < num_y_length< 35:
            ...
            g_car.translation('y', g_y_length)
        else:
            pass
    elif cmd == '20':
        if g_debug:
        ...
        if num_continuous == 1:
            ...
            g_car.perform(g_action_continuous)
